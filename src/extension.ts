...
export function activate(context: vscode.ExtensionContext) {
    ...
    let copyFolderContents1Command = vscode.commands.registerCommand('extension.copyFolderContents1', () => {
        const options: vscode.QuickPickOptions = {
            canPickMany: false,
            placeHolder: 'Enter file extensions to copy (e.g. js, css)',
            ...
            if (extensions) {
                vscode.window.showOpenDialog({
                    ...
                    if (folders && folders.length > 0) {
                        ...
                        const filteredFiles2 = filterFilesByExcludedItems(filteredFiles, '.test.');
                        ...
                        vscode.env.clipboard.writeText(copiedContent).then(() => {
                            vscode.window.showInformationMessage('Folder contents copied to clipboard:  '+ copiedContent.slice(0,50));
                        }, (error) => {
                            vscode.window.showErrorMessage(`Failed to copy folder contents: ${error}`);
                        });
                    }
                });
            }
    ...
    const copyFolderContents2Command = vscode.commands.registerCommand('extension.copyFolderContents2', (uri: vscode.Uri) => {
        if (!uri.fsPath) {
            vscode.window.showErrorMessage('Please select a folder to copy its contents.');
            return;
        }
...
function filterFilesByExcludedItems(files: string[], excludedItems: string): string[] {
